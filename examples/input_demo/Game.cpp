...
void InputDemoState::UpdatePlayerBox(float inDeltaTime, asge::input::InputState const &inInput)
{
...
void InputDemoState::UpdateBackgroundToggle(asge::input::InputState const &inInput)
{
...
void InputDemoState::UpdateBoxSizeFromScroll(asge::input::InputState const &inInput)
{
    m_BoxSize += inInput.GetScrollDelta().y() * kScrollToSize;
    m_BoxSize = std::clamp( m_BoxSize, kMinBoxSize, kMaxBoxSize );
}

void InputDemoState::UpdateCursor(asge::input::InputState const &inInput)
{
    m_CursorPos = inInput.GetMousePosition();
    m_CursorHeld = inInput.IsMouseButtonDown(MouseButton::LEFT);
}

void InputDemoState::UpdateMarks(asge::input::InputState const &inInput)
{
...
InputDemoState::Update(float inDeltaTime, asge::input::InputState const &inInput)
{
...
void InputDemoState::Render(asge::video::IRenderer &inRenderer)
{
...
void InputDemoState::OnSystemEvent([[maybe_unused]] asge::event::SystemEvent const &inSysEvent)
{
...
std::unique_ptr<InputDemoGame::StateType> InputDemoGame::CreateState([[maybe_unused]] int inId)
{
    return std::make_unique<InputDemoState>();
}
