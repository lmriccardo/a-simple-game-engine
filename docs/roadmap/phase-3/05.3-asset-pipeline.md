...
[Phase 2](../phase-2/05.2-asset-pipeline.md) (caching, async streaming).*
...
[01-Core Engine](../phase-1/01.1-core-engine.md)'s configuration hot-reload already wires
...
- **Reuse `FileWatcher`, don't reimplement it.** The config system's hot-reload
  ...
  shape with the cache from [Phase 2](../phase-2/05.2-asset-pipeline.md) as the thing
  being invalidated instead of a config value.

## Tasks
- [ ] Wire `FileWatcher` to invalidate/reload cache entries (from
      [Phase 2](../phase-2/05.2-asset-pipeline.md)) when their source file changes
