...
decoupling from each other. Not MVP: an MVP-scoped game just calls functions
directly. See [README](../README.md). Other phases:
[Phase 3](../phase-3/11.3-scripting-and-events.md) (managed scripting, runtime
reload).*

## Goals
An event dispatcher gameplay systems can use to react to each other without direct
coupling, and native (C++) scripting hooks for entity behavior. Not part of the
MVP — the first phase this system actually needs once it's worth building at all.
...
[01-Core Engine](../phase-1/01.1-core-engine.md)'s `FileWatcher` for change
notifications. What's missing isn't the dispatch primitive, it's a game-event-specific
bus built on top of it (named/typed gameplay events systems can publish and
subscribe to). No scripting/behavior code exists at all yet;
[03-ECS](../phase-1/03.1-entity-component-system.md) stubs a `Script` component
specifically for this phase to fill in.
...
- **Native scripting means a `Script` component invoking a plain C++
  ...
  [03-ECS](../phase-1/03.1-entity-component-system.md) `Script` placeholder becomes
  a real callback hook, not a new language.
- **Managed scripting (Lua/C#/Python) is well past MVP** and stays entirely in
  [Phase 3](../phase-3/11.3-scripting-and-events.md) — it needs a real embedding +
...
[Phase 3](../phase-3/11.3-scripting-and-events.md). A reflection system — not needed
for native callbacks, and [06-Scene Management](../phase-1/06.1-scene-management.md)
is the doc that will define real serialization requirements if one's ever needed.
