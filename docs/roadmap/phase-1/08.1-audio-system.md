...
Other phases: [Phase 2](../phase-2/08.2-audio-system.md) (spatial audio, music
streaming), [Phase 3](../phase-3/08.3-audio-system.md) (DSP effects, reverb zones).*
...
`AudioSource` implementation. [03-ECS Phase 1](03.1-entity-component-system.md)
...
## Design notes
- **Wants [05-Asset Pipeline Phase 1](05.1-asset-pipeline.md)'s
...
- **SDL_audio as the backend**, matching how video ([SDLRenderer](02.1-rendering-primitives.md))
  and input ([SDL_TranslationUnit](04.1-input-system.md)) already build on
...
- **A system, not a singleton `AudioManager` god-object.** Following
  [03-ECS](03.1-entity-component-system.md)'s convention: an `AudioSystem` operating
...
## Tasks
- [x] Audio backend initialization via `SDL_audio` (mirrors how
      [01-Core Engine](01.1-core-engine.md)'s `VideoSystem` initializes the SDL
      video backend)
- [x] WAV/OGG loading, following [05-Asset Pipeline](05.1-asset-pipeline.md)'s
      asset-handle shape
- [x] `AudioSource` component: clip handle, volume, playing state
- [x] `AudioSystem`: plays/stops clips based on `AudioSource` state, same shape as
      `MovementSystem`/`RenderSystem`
- [x] Master/per-source volume control
...
3D/spatial audio, music streaming ([Phase 2](../phase-2/08.2-audio-system.md)); DSP
effects, reverb zones ([Phase 3](../phase-3/08.3-audio-system.md)); a full mixer graph
