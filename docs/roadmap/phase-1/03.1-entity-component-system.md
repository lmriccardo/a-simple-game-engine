...
[Phase 2](../phase-2/03.2-entity-component-system.md) (archetypes, parallel systems).*
...
- **Component storage**: a simple per-type sparse-set / dense-vector store
  ...
  enough entities yet to justify it — see [Phase 2](../phase-2/03.2-entity-component-system.md).
...
- **No serialization or reflection layer yet.** [06-Scene Management](06.1-scene-management.md)
...
- `Sprite` — an `ITexture` handle + optional source `math::Rect`, drawn via the
  existing `DrawTexture` family (see [02-Rendering Primitives](02.1-rendering-primitives.md))
- `Collider` — shape + bounds; feeds [07-Physics](07.1-physics-system.md) once that starts
- `Rigidbody` — velocity/mass; also feeds 07-Physics
- `AudioSource` — feeds [08-Audio](08.1-audio-system.md)
- `Camera` — feeds [10-Rendering System](10.1-rendering-system.md) Step 1
- `Script`/behavior hook — deferred; doesn't mean much beyond a raw function pointer
  until [11-Scripting & Events](../phase-2/11.2-scripting-and-events.md)'s event dispatcher exists
...
[Phase 2](../phase-2/03.2-entity-component-system.md). Serialization specifically is
blocked on [06-Scene Management](06.1-scene-management.md), which doesn't exist yet and
would only get guessed at prematurely. A general `Script`/behavior-component runtime is
blocked on [11-Scripting & Events](../phase-2/11.2-scripting-and-events.md).
