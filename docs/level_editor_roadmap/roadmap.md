...
- [ ] Screen→world inverse transform using the camera/projection math
      already live in the renderer (reuse, don't re-derive).
- [ ] Mouse click hit-tests against each entity's `Transform` position
      (+ sprite bounds if available) to pick a single entity. No multi-select
      yet.
- [ ] ImGui inspector window: if an entity is selected and has a
...
- [ ] Optional at this stage: dragging the entity directly in the viewport
      ...
      of only the ImGui drag fields.
- [ ] "Save" menu item calls the existing save path (`Serializer<Transform>`
      → `TOMLTableView` → file write) — exactly the code the game's save
      path already uses, unmodified.
- [ ] Manually verify: edit position in editor → save → load the file with
...
**Done when:** you can visually confirm collider placement/overlap without
...
**Done when:** a level can be authored from an empty scene, not just an
existing one edited in place.
